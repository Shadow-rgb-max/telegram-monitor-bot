"""
Клиент мониторинга Telegram-каналов на Pyrogram v2 с SOCKS5 прокси.
"""
...
class PyrogramKeywordBot:
    ...
    Основан на Pyrogram v2 с поддержкой SOCKS5 прокси.
    """
    ...
    def _build_client(self) -> Client:
        ...
        proxy = proxy_manager.get_proxy(wait_seconds=15)

        if proxy:
            self.logger.info(f"🔌 Pyrogram Client с SOCKS5 прокси: {proxy.get('hostname')}:{proxy.get('port')}")
        else:
            self.logger.info("🔌 Pyrogram Client без прокси (прямое соединение)")
    ...
    def _ensure_session_schema(self) -> None:
        ...
        try:
            ...
            if pyrogram_utils.MIN_CHANNEL_ID > -1002725157476:
                self.logger.info("🔧 Расширяю диапазон Pyrogram MIN_CHANNEL_ID для поддержки старых channel IDs")
    ...
    async def _resolve_channel(self, channel: str) -> Optional[int]:
        """Разрешает username/ссылку канала в chat_id. Обходит баги Pyrogram v2."""
        channel = channel.strip()

        # Если уже числовой ID
        if channel.lstrip("-").isdigit():
            return int(channel)

        # Приватные ссылки t.me/+xxx → разрешаем через join_chat
        ...
        # Публичные каналы: используем resolve_peer вместо get_chat (обход бага last_update_on)
        try:
            # resolve_peer возвращает InputPeerChannel/InputPeerChat, оттуда берём channel_id
            ...
            self.logger.error(f"❌ Канал {channel} недоступен: {e}")
            return None
        except Exception as e:
            self.logger.error(f"❌ Ошибка разрешения канала {channel}: {e}")
            return None
    ...
    async def _send_startup_test(self):
        try:
            ...
            proxy_info = f"SOCKS5 пул: {stats['pool_size']} прокси" if stats['use_proxy_pool'] else "Прямое соединение"
    ...
    async def start(self):
        ...
        try:
            ...
            self.logger.info("🔌 Подключение к Telegram (Pyrogram)...")
            ...
            if not self._resolved_channels:
                self.logger.error("❌ Ни один канал не разрешён! Проверьте config.ini")
            ...
            if not self._resolved_channels:
                self.logger.error("❌ Ни один канал не доступен! Проверьте config.ini")
                await self._client.stop()
                return

            await self._send_startup_test()

            @self._client.on_message(filters.chat(self._resolved_channels))
